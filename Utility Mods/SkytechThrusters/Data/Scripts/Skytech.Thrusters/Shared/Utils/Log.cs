...
namespace Skytech.Thrusters.Shared.Utils
{
    public static class Log
    {
        ...
        private const string ModName = "Skytech.Thrusters";
        ...
        public static void Init(IMyModContext context)
        {
            ...
            MyLog.Default.WriteLineAndConsole($@"[{ModName}] - Debug log can be found in %AppData%\Roaming\SpaceEngineers\Storage\Skytech.Thrusters.log");
        }
        ...
        public static void Exception(string source, Exception exception, bool fatal = false)
        {
            ...
            _writer?.WriteLine(toWrite);
            _writer?.Flush();
            MyLog.Default.WriteLineAndConsole($"[Skytech.Thrusters] [{(fatal ? "FATAL " : "")}EXCEPTION]\t{source}\n{exception}");
            _currentFileSize += toWrite.Length;
        ...
        private class CustomCrashModContext : IMyModContext
        {
            ...
            public static ModCrashedException GenerateException(string source, Exception ex)
            {
                var context = new CustomCrashModContext(Log._modContext,
                    ...
                    "Mod: %AppData%\\SpaceEngineers\\Storage\\Skytech.Thrusters.log \n" +
