...
using VRage.ModAPI;
using VRage.Utils;
using static Skytech.Thrusters.Shared.Utils.IniConfig;
    ...
    public static class GlobalData
    {
        ...
        internal static bool CheckShouldLoad()
        {
            foreach (var mod in MyAPIGateway.Session.Mods)
            {
                ...
                if (modIdFormatted.Contains("skytech") && modIdFormatted.Contains("thrusters"))
                {
                    Killswitch = true;
                    MyLog.Default.WriteLineAndConsole($"[Skytech Thrusters] Found local Skytech.Thrusters version \"{mod.GetPath()}\" - cancelling init and disabling mod. My ModId: {SharedMain.I.ModContext.ModId}");
                    return false;
                }
            }
